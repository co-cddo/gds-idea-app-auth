...
def test_get_auth_user_falls_back_to_cache_on_missing_headers(
    mock_streamlit, streamlit_auth
):
    """get_auth_user uses cached user when headers are missing (WebSocket reconnect)"""
    # Create mock user with future expiration
    future_exp = datetime.now() + timedelta(hours=1)
    mock_user = User.create_mock(groups=["developers"])
    ...
    # Simulate missing headers (WebSocket reconnection)
    with patch.object(
        streamlit_auth,
...
def test_get_auth_user_fails_when_no_cache_and_no_headers(
    mock_streamlit, streamlit_auth
):
    """get_auth_user stops when headers missing and no cached user"""
    # No cached user, headers missing
    with patch.object(
        streamlit_auth,
        "_get_user_from_headers",
        side_effect=MissingTokenError("No headers"),
        ...
        # Verify error shown
        mock_streamlit.error.assert_called_once()
        assert "initialization failed" in mock_streamlit.error.call_args[0][0].lower()
